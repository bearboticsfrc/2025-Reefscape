...
public class FieldPositions {
  ...
  /**
   * Enum for the reef reference points. Points follow a counter-clockwise order starting with the
   * lowest and furthest left on the blue side. Starting with the highest and furthest right on the
   * red side. this way the points will function as way points on both the red and blue sides
   */

  // since this class will only need to be constructed
  public enum ReefReferencePoint {
    REF_POINT_ONE(new double[] {DIST_TO_PERIM, (FIELD_WIDTH / 2) - (REEF_SIDE_LENG / 2)}),
    ...
    public Pose2d getPose() {
      ...
      if (AllianceColor.getAlliance() == Alliance.Red) {
        return new Pose2d(
            FIELD_LENGTH - coordinates[0], FIELD_WIDTH - coordinates[1], new Rotation2d());
      }
